        ...
        for s_idx, f_list in assignments.items(): 
            ...
            while f_list:
                ...
                if not possible.empty:
                    if area_avail >= possible.iloc[0]['Gaseous Footprint (acres)']: #Apply Gaseous Footprint 
                        active_count += 1
...
if res.F is not None:
        ...
        for s_idx in selected_indices:
            ...
            while f_list:
                ...
                if not possible.empty and demand <= MAX_CAPACITY:
                    if area_avail >= possible.iloc[0]['Gaseous Footprint (acres)']:
    ...
    if winner['details']:
        pd.DataFrame(winner['details']).to_csv(OUTPUT_DIR2 / 'final_gasbalanced_sites_details.csv', index=False)
        print(f"Success! Data exported to: {OUTPUT_DIR2 / 'final_gasbalanced_sites_details.csv'}")
...
if len(gen_callback.all_generation_data) > 0:
    ...
    plt.savefig(OUTPUT_DIR / 'gaseous_final_optimization_process_map.png', dpi=300)
    plt.show()
...
if res.F is not None:
    ...
    plt.legend()
    plt.savefig(OUTPUT_DIR / 'gaseous_final_pareto_with_selection.png', dpi=300)
    plt.show()
...
if res.F is not None:
    ...
    # 4. Save and Show
    plt.suptitle('Gaseous Tube Trailer Supplied Hydrogen Infrastructure: Pareto Front Diagnostic (6 Perspectives)', fontsize=20, y=0.98)
    plt.savefig(OUTPUT_DIR / 'gaseous_final_detailed_objective_analysis.png', dpi=300, bbox_inches='tight')
    print(f"[Visual] Analysis plots saved to: {OUTPUT_DIR / 'gaseous_final_detailed_objective_analysis.png'}")
    plt.show()