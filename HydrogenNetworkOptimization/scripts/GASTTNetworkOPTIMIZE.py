...
if 'summary_rows' in locals() and len(summary_rows) > 0:
    ...
    plt.suptitle(f'{FOOTPRINT_TYPE.capitalize()} Tube-Trailer Supplied Hydrogen Refueling Infrastructure: Performance Across Capacities', fontsize=15, fontweight='bold', y=1.02)
    plt.tight_layout()
...
if res.F is not None:
    ...
    inf_result1 = find_inflection_point(X1, Y1, tolerance_ratio=0.50)
    if inf_result1 is not None:
    ...
    inf_result3 = find_inflection_point(X3, Y3, tolerance_ratio=0.50)
    if inf_result3 is not None:
    ...
    plt.suptitle(f'{FOOTPRINT_TYPE.capitalize()} Tube-Trailer Supplied Hydrogen Infrastructure: Pareto Front Breakdown', fontsize=16, fontweight='bold', y=1.02)
    plt.tight_layout()
