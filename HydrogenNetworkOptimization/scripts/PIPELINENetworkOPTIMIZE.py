...
FOOTPRINT_COLUMN_MAP = {
    ...
    'gaseous': 'Gaseous Footprint (acres)',  # Adjust string if your CSV header differs slightly
...
if 'summary_rows' in locals() and len(summary_rows) > 0:
    ...
    def find_curve_inflection(x_smooth, y_smooth, tolerance_ratio=0.05):
    ...
    inf_result1 = find_curve_inflection(X_smooth, Y1_smooth, tolerance_ratio=0.05)
    if inf_result1 is not None:
        ax1.scatter(inf_result1[0], inf_result1[1], color='red', marker='*', s=200, edgecolors='black', zorder=5)
        
        # --- ADJUST THE VALUES IN xytext=(X, Y) TO MOVE THE LABEL ---
        # (0, 43) means 0 points horizontally, and 43 points directly ABOVE the star.
        ax1.annotate(f"Inflection: {int(inf_result1[0]):,} kg", 
                     xy=inf_result1, 
                     xytext=(-75, 25),          # <-- Increase to push higher up, use negative to drop below
    ...
    inf_result2 = find_curve_inflection(X_smooth, Y2_smooth, tolerance_ratio=0.05)
    if inf_result2 is not None:
        ax2.scatter(inf_result2[0], inf_result2[1], color='red', marker='*', s=200, edgecolors='black', zorder=5)
        
        # --- ADJUST THE VALUES IN xytext=(X, Y) TO MOVE THE LABEL ---
        # (0, 43) means 0 points horizontally, and 43 points directly ABOVE the star.
        ax2.annotate(f"Inflection: {int(inf_result2[0]):,} kg", 
                     xy=inf_result2, 
                     xytext=(0,-55),          # <-- Increase to push higher up, use negative to drop below
                     textcoords="offset points", 
                     color='red', 
                     fontweight='bold', 
                     ha='center', 
    ...
    inf_result3 = find_curve_inflection(X_smooth, np.polyval(p3, X_smooth), tolerance_ratio=0.05)
    if inf_result3 is not None:
        ...
        # --- ADJUST THE VALUES IN xytext=(X, Y) TO MOVE THE LABEL ---
        # (0, 43) means 0 points horizontally, and 43 points directly ABOVE the star.
        ax3.annotate(f"Inflection: {int(inf_result3[0]):,} kg", 
                     xy=inf_result3, 
                     xytext=(-70,0),          # <-- Increase to push higher up, use negative to drop below
                     textcoords="offset points", 
                     color='red', 
                     fontweight='bold', 
                     ha='center', 
...
if res.F is not None:
    ...
    inf_result1 = find_inflection_point(X1, Y1, tolerance_ratio=0.7)
    if inf_result1 is not None:
    ...
    inf_result2 = find_inflection_point(X2, Y2, tolerance_ratio=0.05)
    if inf_result2 is not None:
    ...
    inf_result3 = find_inflection_point(X3, Y3, tolerance_ratio=0.7)
    if inf_result3 is not None:
